...
export default function ViewUserPage() {
  ...
  const loadUser = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/dashboard/users/get/${id}`);
  ...
  if (!user) {
    return (
      <div className="p-6 text-[#2c3338]">
        ...
        <Link href="/dashboard/utilizadores" className="text-[#2271b1] hover:underline mt-2 inline-block">← Voltar aos Utilizadores</Link>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 text-[#2c3338] max-w-[900px]">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[13px] text-[#50575e] mt-1">
            <Link href="/dashboard/utilizadores" className="text-[#2271b1] hover:underline">Utilizadores</Link>
            {' '}&rsaquo; Ver perfil
          </p>
        </div>
        <Link
          href={`/dashboard/utilizadores/editar/${user.id}`}
        ...
        <table className="w-full border-collapse">
          <tbody>
            ...
            <tr>
              ...
              <td className="p-3 text-[13px]">
                <Link href={`/dashboard/noticias?autor=${user.username}`} className="text-[#2271b1] hover:underline flex items-center gap-1">
      ...
      <div className="mt-4 flex items-center gap-3">
        <Link href={`/dashboard/utilizadores/editar/${user.id}`} className="h-8 px-4 flex items-center bg-[#2271b1] text-white text-[13px] font-semibold rounded-md hover:bg-[#135e96] transition-all">
          Editar utilizador
        </Link>
        {!user.isAdmin && (
