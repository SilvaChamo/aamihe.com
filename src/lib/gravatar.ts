...
export function getGravatarUrl(email?: string | null, size = 150) {
  const normalized = (email || '').trim().toLowerCase();
  if (!normalized) {
    return `https://secure.gravatar.com/avatar/ad516503a11cd5ca435acc9bb6523536?s=${size}&d=mp&r=g`;
  }
  const hash = md5(normalized);
  return `https://secure.gravatar.com/avatar/${hash}?s=${size}&d=mp&r=g`;
}
